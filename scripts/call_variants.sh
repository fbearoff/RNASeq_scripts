...
#Notes
    ...
    #Supply SAMPLES_FILE as genotype:sample per line, genotype must equal file prefix

##########CONFIG############
DATA_FOLDER="/home/frank/data/ALS_RNASeq2/20180302blankenhorn"  #location of read files
SAMPLES_FILE="/home/frank/data/ALS_RNASeq2_samples.txt"
OUTPUT_FOLDER="/scratch/frank/ALS_RNASeq2_call_variants"
...
elif [ ! -f "$IGVTOOLS_LOCATION" ]; then
    echo "IGVTools JAR binary not present, exiting!" && exit
fi
...
if [ ! -f "${REF_LOCATION%.fa}".dict ]; then
    java -jar "$PICARD_LOCATION" CreateSequenceDictionary \
        R= "$REF_LOCATION" \
        O= "${REF_LOCATION%.fa}".dict
...
STAR \
    ...
    --genomeDir "$OUTPUT_FOLDER"/genome \
    --genomeFastaFiles "$REF_LOCATION" \
    --sjdbGTFfile "$REF_ANNOTATION" \
    --outFileNamePrefix "$OUTPUT_FOLDER"/genome/

#1-Pass map reads against genome index
mkdir "$OUTPUT_FOLDER"/1-pass/ || exit
for sample in ${aa_samples[@]}; do
    echo ">1st-pass mapping sample ${sample}"
    mkdir "$OUTPUT_FOLDER"/1-pass/"$sample"
    STAR \
        --runThreadN "$THREADS" \
        --genomeDir "$OUTPUT_FOLDER"/genome \
        --readFilesIn "$DATA_FOLDER"/"${sample}.R1.fq.gz" "$DATA_FOLDER"/"${sample}.R2.fq.gz" \
        --readFilesCommand gunzip -c \
        --outFileNamePrefix "$OUTPUT_FOLDER"/1-pass/"${sample}"/ \
...
for genotype in "${!aa_samples[@]}"; do
    ...
    for sample in $(echo "${aa_samples[$genotype]}"); do
        ...
        echo ">2nd-pass mapping sample ${sample}"
        STAR \
            --runThreadN "$THREADS" \
            --genomeDir "$OUTPUT_FOLDER"/genome \
            --readFilesIn "$DATA_FOLDER"/"${sample}.R1.fq.gz" "$DATA_FOLDER"/"${sample}.R2.fq.gz" \
            --readFilesCommand gunzip -c \
            --outFileNamePrefix "$OUTPUT_FOLDER"/2-pass_output/"$sample"/ \
            --outSAMattrRGline ID:"${sample}" SM:"${sample}" LB:"${sample}" PL:illumina PU:nextseq \
            --sjdbFileChrStartEnd "$OUTPUT_FOLDER/1-pass/$genotype.SJ.out.tab" \
            --outStd SAM \
            |samblaster \
            |sambamba view \
            -t "$THREADS" \
            ...
            -t "$THREADS" \
            -l 9 /dev/stdin \
            -o "$OUTPUT_FOLDER"/2-pass_output/"$sample"/"$sample".sorted.bam
    done
done

#Reassign MAPQ scores and split BAMs
mkdir -p "$OUTPUT_FOLDER"/vc_BAMs/split || exit
echo ">Recalibrating and splitting BAMs for all samples"
parallel -j "$NJOBS" 'java -jar '"$GENOMEANALYSISTK_LOCATION"' \
    -T SplitNCigarReads -R '"$REF_LOCATION"' \
    -I '"$OUTPUT_FOLDER"'/2-pass_output/{}/{}.sorted.bam \
    -o '"$OUTPUT_FOLDER"'/vc_BAMs/split/{}.split.bam \
    -rf ReassignOneMappingQuality \
    -RMQF 255 \
    -RMQT 60 \
    -U ALLOW_N_CIGAR_READS' ::: $(echo "${aa_samples[@]}")
...
for sample in ${aa_samples[@]}; do
    ...
    java -jar "$GENOMEANALYSISTK_LOCATION" \
        -nt "$THREADS" \
        -T RealignerTargetCreator \
        -R "$REF_LOCATION" \
        -known "$KNOWN_SITES" \
        -I "$OUTPUT_FOLDER"/vc_BAMs/split/"${sample}".split.bam \
        -o "$OUTPUT_FOLDER"/vc_BAMs/realign/"${sample}".intervals
#Convert to BED for IGV
    awk -F '[:-]' 'BEGIN { OFS = "\t" } { if( $3 == "") { print $1, $2-1, $2 } else { print $1, $2-1, $3}}' \
        "$OUTPUT_FOLDER"/vc_BAMs/realign/"${sample}".intervals > "$OUTPUT_FOLDER"/vc_BAMs/realign/"${sample}".bed
...
parallel -j "$NJOBS" 'java -Djava.io.tmpdir=/tmp -jar '"$GENOMEANALYSISTK_LOCATION"' \
    -T IndelRealigner \
    -R '"$REF_LOCATION"' \
    -targetIntervals '"$OUTPUT_FOLDER"'/vc_BAMs/realign/{}.intervals \
    -known '"$KNOWN_SITES"' \
    -I '"$OUTPUT_FOLDER"'/vc_BAMs/split/{}.split.bam \
    -o '"$OUTPUT_FOLDER"'/vc_BAMs/realign/{}.indelrealigner.bam' ::: $(echo "${aa_samples[@]}")
...
for sample in ${aa_samples[@]}; do
    ...
    java -jar "$GENOMEANALYSISTK_LOCATION" \
        ...
        -T BaseRecalibrator \
        -R "$REF_LOCATION" \
        -I "$OUTPUT_FOLDER"/vc_BAMs/realign/"${sample}".indelrealigner.bam \
        -knownSites "$KNOWN_SITES" \
        -o "$OUTPUT_FOLDER"/base_recalibration/"${sample}".recal_data.table
    java -jar "$GENOMEANALYSISTK_LOCATION" \
        -nct "$THREADS" \
        -T BaseRecalibrator \
        -R "$REF_LOCATION" \
        -I "$OUTPUT_FOLDER"/vc_BAMs/realign/"${sample}".indelrealigner.bam \
        -knownSites "$KNOWN_SITES" \
        -BQSR "$OUTPUT_FOLDER"/base_recalibration/"${sample}".recal_data.table \
        -o "$OUTPUT_FOLDER"/base_recalibration/"${sample}".post_recal_data.table
    java -jar "$GENOMEANALYSISTK_LOCATION" \
        -T AnalyzeCovariates \
        -R "$REF_LOCATION" \
        -before "$OUTPUT_FOLDER"/base_recalibration/"${sample}".recal_data.table \
        -after "$OUTPUT_FOLDER"/base_recalibration/"${sample}".post_recal_data.table \
        -plots "$OUTPUT_FOLDER"/base_recalibration/"${sample}".recalibration_plots.pdf
    java -jar "$GENOMEANALYSISTK_LOCATION" \
        -nct "$THREADS" \
        -T PrintReads \
        -R "$REF_LOCATION" \
        -I "$OUTPUT_FOLDER"/vc_BAMs/realign/"${sample}".indelrealigner.bam \
        -BQSR "$OUTPUT_FOLDER"/base_recalibration/"${sample}".recal_data.table \
        -o "$OUTPUT_FOLDER"/base_recalibration/"${sample}".bam \
        --bam_compression 9
done
...
parallel -j "$NJOBS" 'java -jar '"$GENOMEANALYSISTK_LOCATION"' \
    -T HaplotypeCaller \
    -R '"$REF_LOCATION"' \
    -I '"$OUTPUT_FOLDER"'/base_recalibration/{}.bam \
    --dontUseSoftClippedBases \
    --dbsnp '"$KNOWN_SITES"' \
    -ERC GVCF \
    -stand_call_conf 20 \
    -o '"$OUTPUT_FOLDER"'/called_variants/{}.g.vcf' ::: $(echo "${aa_samples[@]}")
...
for genotype in "${!aa_samples[@]}"; do
    ...
    java -jar "$GENOMEANALYSISTK_LOCATION" \
        -T VariantFiltration \
        -R "$REF_LOCATION" \
        -V "$OUTPUT_FOLDER"/called_variants/"${genotype}".vcf \
        -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" \
        -o "$OUTPUT_FOLDER"/called_variants/"${genotype}".filtered.vcf
done
