...
#Notes
    ...
    #Supply SAMPLES_FILE as genotype:sample per line

##########CONFIG############
DATA_FOLDER="/home/frank/data/ALS_RNASeq/blankenhornRNAseq9samples"  #location of read files
SAMPLES_FILE="/home/frank/data/ALS_samples.txt"
OUTPUT_FOLDER="/scratch/frank/ALS_RNASeq_call_variants"
...
for sample in ${aa_samples[@]}; do
    ...
    STAR --runThreadN "$THREADS" --genomeDir "$OUTPUT_FOLDER"/genome --readFilesIn "$DATA_FOLDER"/"${sample}.R1.fq.gz" "$DATA_FOLDER"/"${sample}.R2.fq.gz" --readFilesCommand gunzip -c --outFileNamePrefix "$OUTPUT_FOLDER"/1-pass/"${sample}"/
done
...
for genotype in "${!aa_samples[@]}"; do
    ...
    for sample in $(echo "${aa_samples[$genotype]}"); do
        ...
        STAR --runThreadN "$THREADS" --genomeDir "$OUTPUT_FOLDER"/genome --readFilesIn "$DATA_FOLDER"/"${sample}.R1.fq.gz" "$DATA_FOLDER"/"${sample}.R2.fq.gz" --readFilesCommand gunzip -c --outFileNamePrefix "$OUTPUT_FOLDER"/2-pass_output/"$sample"/ --outSAMattrRGline ID:"${sample}" SM:"${sample}" LB:"${sample}" PL:illumina PU:nextseq --sjdbFileChrStartEnd "$OUTPUT_FOLDER/1-pass/$genotype.SJ.out.tab" --outStd SAM |samblaster|sambamba view -t "$THREADS" -S /dev/stdin -f bam -l 0 |sambamba sort -t "$THREADS" -l 9 /dev/stdin -o "$OUTPUT_FOLDER"/2-pass_output/"$sample"/"$sample".sorted.bam
    done
done
...
parallel -j "$NJOBS" 'java -jar '"$GENOMEANALYSISTK_LOCATION"' -T SplitNCigarReads -R '"$REF_LOCATION"' -I '"$OUTPUT_FOLDER"'/2-pass_output/{}/{}.sorted.bam -o '"$OUTPUT_FOLDER"'/vc_BAMs/split/{}.split.bam -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS' ::: $(echo "${aa_samples[@]}")
...
parallel -j "$NJOBS" 'java -Djava.io.tmpdir=/tmp -jar '"$GENOMEANALYSISTK_LOCATION"' -T IndelRealigner -R '"$REF_LOCATION"' -targetIntervals '"$OUTPUT_FOLDER"'/vc_BAMs/realign/{}.intervals -known '"$KNOWN_SITES"' -I '"$OUTPUT_FOLDER"'/vc_BAMs/split/{}.split.bam -o '"$OUTPUT_FOLDER"'/vc_BAMs/realign/{}.indelrealigner.bam' ::: $(echo "${aa_samples[@]}")
...
parallel -j "$NJOBS" 'java -jar '"$GENOMEANALYSISTK_LOCATION"' -T HaplotypeCaller -R '"$REF_LOCATION"' -I '"$OUTPUT_FOLDER"'/base_recalibration/{}.bam --dontUseSoftClippedBases --dbsnp '"$KNOWN_SITES"' -ERC GVCF -stand_call_conf 20 -o '"$OUTPUT_FOLDER"'/called_variants/{}.g.vcf' ::: $(echo "${aa_samples[@]}")
